...
interface LoggerApi {
    ...
    @GET("logs/shop")
    suspend fun getLogsShop(@Query(Constants.HttpRequestKeys.ID_FACTORY) idFactory: Long,
                            @Query(Constants.HttpRequestKeys.ID_SHOP) idShop: Long,
                            @Query(Constants.HttpRequestKeys.ID_TRIP) idTrip: Long): BaseResponse<List<LogShopModel>>
