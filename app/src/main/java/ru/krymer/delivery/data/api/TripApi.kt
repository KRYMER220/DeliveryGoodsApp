...
interface TripApi {
    @POST("trip/create")
    suspend fun add(@Body trip: TripRequest): BaseResponse<TripModel>

    @GET("routes/by/trips")
    suspend fun getRoutesByTrip(@Query(Constants.HttpRequestKeys.ID_FACTORY) idFactory: Long): BaseResponse<List<TripModel>>

    @POST("trip/update")
    suspend fun update(@Body trip: TripRequest): BaseResponse<TripModel>
